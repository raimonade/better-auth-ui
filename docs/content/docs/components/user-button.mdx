...
### Role-Based Multi-Session Access Control

You can restrict multi-session functionality (account switching and adding accounts) to admin+ users only by setting the `restrictMultiSessionToAdmins` prop to `true`. This is useful in organization contexts where you want to limit multi-session capabilities to administrators.
...
export default function Example() {
  return (
    <UserButton 
      restrictMultiSessionToAdmins={true}
    />
  )
}
```
...
When enabled:
- **Organization owners and admins**: Can see and use multi-session features (switch accounts, add accounts)
- **Regular members**: Multi-session options are hidden from the dropdown menu
- **Users not in an organization**: Multi-session features remain available (no restriction)
